...
/**
 * TLoD-canonical per-level stat row for Lavitz / Albert (shared
 * column in the source tables — Albert inherits Lavitz's Jade
 * Dragoon Spirit + archetype). Index 0 = LV 1, index 59 = LV 60.
 * Exported so `albert.ts` can reuse without duplicating 60 rows.
 */
export const LAVITZ_STATS_BY_LEVEL: ReadonlyArray<CharacterLevelRow> = [
...
export const LAVITZ: CharacterDef = {
  ...
  // Canonical TLoD addition unlock schedule (from the stat table's
  // rightmost column): Harpoon at LV1, Spinning Cane at LV9, Rod
  // Typhoon at LV18, Gust of Wind Dance at LV32, Flower Storm at
  // LV50. None declared in ADDITIONS yet — consumers filter with
  // `slug in ADDITIONS`.
