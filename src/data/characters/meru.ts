/**
 * Meru — TLoD's Blue-Sea Dragoon. Hammer-wielding melee with high
 * agility (fast atkSpeed, snappy movement) and a water/wind magical
 * leaning.
 *
 * Now wired with the canonical TLoD XP curve. Stat table is still
 * procedural until the canonical row lands — swap the
 * `placeholderStatsByLevel(…)` call below with a literal array
 * when on hand.
 *
 * Sprite is a single full-body pose (256×377) reused for idle /
 * attack / defend until pose variants are generated.
 */
import type { CharacterDef, CharacterLevelRow } from './types';

/** TLoD-canonical per-level stat row for Meru. Index 0 = LV 1. */
const MERU_STATS_BY_LEVEL: ReadonlyArray<CharacterLevelRow> = [
...
/**
 * Cumulative XP to reach each level — TLoD canonical "Meru" column.
 * Levels 1-17 back-filled with Dart's values (Meru joins the party
 * at LV18 in the original, but Survival starts at LV1 universally
 * so we need a threshold there).
 */
...
export const MERU: CharacterDef = {
  ...
  // ADDITIONS yet — consumers filter by `slug in ADDITIONS`.
