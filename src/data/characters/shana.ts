 ...
 * Now wired with the canonical TLoD XP curve (shared column with
 * Miranda — they're the same archetype in TLoD's tables). Stat
 * table is still procedural until the canonical row lands; the
 * swap is a literal `ReadonlyArray<CharacterLevelRow>` drop-in
 * replacing the `placeholderStatsByLevel(…)` call below.
 *
 * Sprite aliases resolve to the real Shana PNGs (idle + attack).
 * Defend reuses the idle until a dedicated guard pose ships.
 */
import type { CharacterDef, CharacterLevelRow } from './types';

/**
 * TLoD-canonical per-level stat row for Shana / Miranda (shared
 * column in the source tables). Index 0 = LV 1, index 59 = LV 60.
 * Exported so `miranda.ts` can reuse the same table without
 * duplicating 60 rows in two files.
 */
export const SHANA_STATS_BY_LEVEL: ReadonlyArray<CharacterLevelRow> = [
...
/**
 * Cumulative XP to reach each level — TLoD canonical "Shana /
 * Miranda" column. Levels 1-4 back-filled with Dart's values
 * (Shana joins the party at LV5 in the original, but Survival
 * starts at LV1 universally so we need a threshold there).
 */
...
export const SHANA: CharacterDef = {
  ...
  actionStats: {
    ...
    base: {
      ...
      atkSpeed: 1.0,
      // Bow range — ~ 3× Dart's melee 80 px so kiting feels natural.
      // CombatSystem's ranged branch reads this for projectile spawn.
      range: 240,
      aggroRange: 0,
      attackHit: 95,
      magicHit: 100,
  ...
  // Shana uses Items + White-Silver Dragoon spells in TLoD, not
  // Additions. Empty for now; spells will plug in via a future
  // character-aware unlock map on SpellSystem.
